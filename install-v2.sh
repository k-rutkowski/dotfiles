...
install_desktop() {
	...
	$run $sudox pacman -S --noconfirm neovim tar less bc btop cifs-utils net-tools bind git git-lfs base-devel cmake make clang ninja
	$run $sudox pacman -S --noconfirm man tldr python3 curl wget nodejs npm tmux ranger imagemagick os-prober xdotool xclip entr fastfetch jq lsd bat zoxide ripgrep fd git-delta dust rsync trash-cli lazygit chafa socat
	...
	# music
	$run flatpak install -y flathub com.mastermindzh.tidal-hifi
